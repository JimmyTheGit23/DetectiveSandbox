...
### 8.11 P0 制作强制约束（2026-06-01 归档）

> 本节作为当前制作轮的硬约束。若后续剧情、表格或引擎实现与本节冲突，优先服从本节。

#### 8.11.1 数据与命名硬规则

1. 角色显示名统一为“沈清月”，不得再写“沈青月”。
2. 角色 ID 统一为 `shen_qingyue`。
3. CSV 为主要数据源；涉及对峙的运行时 JSON 必须由 `data/case_tables/prologue_ferry/` 编译生成。
4. 对峙文本短期只使用现有字段：`intro_dialogue`、`preamble`、`press`、`break_dialogue`、`transition_dialogue`、`fail_dialogue`、`wrong_reactions`、`victory_dialogue`、`defeat_dialogue`。
5. 不使用未落地字段如 `speaker_override`、`before_press`、`before_present`；这些仅作为 P1 引擎增强项。
6. `confrontation_final` 必须走“玩家机制胜利 → 香囊赝品翻盘 → 剧情败局”，不得改成玩家信心归零失败。

#### 8.11.2 对峙体验硬规则

1. 沈清月前期是理性讼师，不是一眼反派；所有反驳必须先站在“证据规则”上。
2. 每轮证词至少包含一个“沈清月框定/反压/补救”的节点，优先放在 `preamble`、`press`、`break_dialogue`、`transition_dialogue`。
3. 阿贵/老范阶段的沈清月保护逻辑必须分三层：表面公正 → 合理化证人说法 → 棋子快崩时控场。
4. 终局沈清月不低于五层防线：无动机、钱不是我捞的、不认识共犯、在场不等于策划、没有直接物证。
5. 凌瑶每场对峙至少一次指出沈清月“帮忙时机太巧”或“反驳看似合理但在偷换概念”。
6. 错误举证反馈中，沈清月可以嘲讽，但不得提前暴露主谋身份。

#### 8.11.3 当前 P0 落地顺序

| # | 任务 | 目标 |
|---|---|---|
| 1 | 沈清月立绘比例校准 | 先消除视觉违和，缩小约 15%~20% |
| 2 | 阿贵/老范对峙加厚 | 用现有 dialogue 队列增强三方攻防 |
| 3 | 终局扩成五轮防线 | 把沈清月做成真正 Final Boss |
| 4 | 编译运行时数据 | 让 CSV 修改同步到 `case.json` |
| 5 | P1 另开引擎增强 | 再做操作前交锋钩子与右侧对手立绘 |
