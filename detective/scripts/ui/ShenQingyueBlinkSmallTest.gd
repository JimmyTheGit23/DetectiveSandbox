extends Control
## 方案 C 验证 (沈清月): ROI 大小的小贴图眼层 + 多帧 alpha 曲线眨眼。
##
## 对比 DynamicPortraitTestV2 沈清月版:
##   - 眼层贴图: 848x1264 全帧 → 305x34 ROI 小图(美术量约 1/100)
##   - 底部硬切: y>311 的"脸颊溢出 18 行"已在生成阶段砍掉, 消除眨眼时
##     脸颊皮肤变暗的不自然感
##   - 眼层定位: 通过 layer_rect 显式指定 ROI 在画布中的位置 (315, 278)
##
## 按 B 键手动眨眼; 自动每 2.5~5s 眨一次。

const BODY_PATH := "res://assets/cn/portraits/prologue_shen_qingyue.png"
const EYES_HALF_PATH := "res://assets/cn/portraits/anim_layers/shen_qingyue/small/eyes_half.png"
const EYES_CLOSED_PATH := "res://assets/cn/portraits/anim_layers/shen_qingyue/small/eyes_closed.png"

## ROI 在源图(848x1264)中的位置 —— 与 crop_blink_to_small.py 配置一致
const EYE_ROI_IN_SOURCE := Rect2(315, 278, 305, 34)
...
func _build_ui() -> void:
	...
	var title := Label.new()
	title.text = "沈清月眨眼 · 方案C (ROI 小贴图, 已硬切脸颊溢出)"
