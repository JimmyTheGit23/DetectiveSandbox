...
from portrait_generation_spec import NPC_KNEE_UP_SPEC, autocrop_rgba
...
def main():
    ...
    ap.add_argument("--target-width", type=int, default=NPC_KNEE_UP_SPEC.canvas_width, help=f"目标宽度（默认 {NPC_KNEE_UP_SPEC.canvas_width}）")
    ap.add_argument("--target-height", type=int, default=NPC_KNEE_UP_SPEC.canvas_height, help=f"目标高度（默认 {NPC_KNEE_UP_SPEC.canvas_height}）")
    ap.add_argument("--padding", type=int, default=NPC_KNEE_UP_SPEC.crop_padding, help=f"裁切边距像素（默认 {NPC_KNEE_UP_SPEC.crop_padding}）")
    args = ap.parse_args()
    ...
    img = autocrop_rgba(img, padding=args.padding)
